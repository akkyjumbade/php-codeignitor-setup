...
fetch('/app.json').then(res => res.json()).then(res => {
   res.icons.map(icon => {
      let iconSrc = icon.src
      urlsToCache = [...urlsToCache, iconSrc]
   })
...
//       clients.openWindow('/index.php/admin?ref=push_notification&tag='+tag)
//       return
//    }
//    return notify
// });
